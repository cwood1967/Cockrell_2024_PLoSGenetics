...
def process(filename, image, cnn, is_cellpose,
            frame=0,
...
## background subtract
    ti0 = time.time()
    f = bgs(f, 25) #ndi.white_tophat(f, (0, 25, 25, 0))
    ti1 = time.time()
    ...
    tc0 = threshold_otsu(g0)
    tc1 = threshold_otsu(g1)
    ...
    if tx0.shape[0] < 3:
        ...
        selmz = 3
    tx0 = ndi.morphology.binary_opening(tx0, structure=np.ones((selmz,3,3))) #.astype(np.int32)
   
    #tx0 = ndi.morphology.binary_erosion(tx0).astype(np.int32)
    tx1 = ndi.morphology.binary_opening(tx1, structure=np.ones((1,3,3))).astype(np.int32)
    stx0 = (tx0.max(), tx1.max(), tx0.sum(), tx1.sum())
    
    no_red = np.where(tx1 - tx0 > 0, 1, 0)
    combined_mask = tx0 + tx1
    combined_mask = np.where(combined_mask > 0, 1, 0)

    rdna_mask = np.where(tx0 + tx1 > 1, 1, 0)
    fmask = np.stack((tx0, combined_mask, no_red, rdna_mask))
...
def run(folder, pkl_name, modelfile, size=400, probability=0.95,
        just_green_min_size=200, green_in_red_min_size=64,
        globpattern='*.nd2', channels=[0,1,2],
